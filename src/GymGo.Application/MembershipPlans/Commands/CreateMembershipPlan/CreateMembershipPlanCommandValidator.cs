...
public sealed class CreateMembershipPlanCommandValidator : AbstractValidator<CreateMembershipPlanCommand>
{
    public CreateMembershipPlanCommandValidator()
    {
        ...
        // Si días son rígidos, debe haber al menos un día marcado
        RuleFor(x => x)
            .Must(x => !x.FixedDays || AnyDayMarked(x))
            .WithMessage("Si el plan tiene días rígidos, debe marcar al menos un día de la semana.")
            .WithName("FixedDays");

        // Si días son rígidos, la cantidad de días marcados debe coincidir con DaysPerWeek
        RuleFor(x => x)
        ...
        // Si horario no es libre, HoraDesde y HoraHasta son obligatorias
    ...
    private static bool AnyDayMarked(CreateMembershipPlanCommand x) =>
        x.Monday || x.Tuesday || x.Wednesday || x.Thursday || x.Friday || x.Saturday || x.Sunday;

    private static int CountDays(CreateMembershipPlanCommand x)
    {
        var count = 0;
        if (x.Monday)    count++;
        if (x.Tuesday)   count++;
