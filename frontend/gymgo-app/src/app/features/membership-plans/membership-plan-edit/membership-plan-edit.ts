...
@Component({
  ...
  templateUrl: './membership-plan-edit.html',
})
export class MembershipPlanEditComponent implements OnInit {
  private readonly fb          = inject(FormBuilder);
  private readonly router      = inject(Router);
  private readonly planService = inject(MembershipPlanService);
  private readonly dialog      = inject(DialogService);

  /** Enlazado desde la ruta /membership-plans/:id/edit via withComponentInputBinding() */
  ...
  readonly plan        = signal<MembershipPlanDto | null>(null);
  ...
  loadPlan(): void {
    ...
    this.planService.getMembershipPlanById(this.id()).subscribe({
      next: (dto) => {
        this.plan.set(dto);
        this.form.patchValue({
          ...
          sunday:         dto.sunday,
          freeSchedule:   dto.freeSchedule,
          timeFrom:       toTimeInput(dto.timeFrom),
          timeTo:         toTimeInput(dto.timeTo),
          allowsFreezing: dto.allowsFreezing,
        });
        this.loadingPlan.set(false);
      },
      error: () => {
        this.errorLoad.set('No se pudo cargar el plan. Intenta nuevamente.');
        this.loadingPlan.set(false);
      },
    });
  }
      ...
      .subscribe({
        next: async () => {
          ...
          await this.dialog.success(
            '¡Plan actualizado!',
            `El plan "${raw.name}" fue actualizado exitosamente.`,
          );
  ...
  private parseError(err: HttpErrorResponse): string {
    console.error('[MembershipPlanEdit] error HTTP', err.status, err.error);
    ...
    if (err.status === 404) return 'El plan no fue encontrado.';
    ...
    return `Error ${err.status}: no se pudo actualizar el plan. Intenta nuevamente.`;
  }
}
